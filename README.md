...
`runa runs show <id>` looks the id up in the app's configured `RunStore` and prints `timeline(run)` (manifesto §11); it only finds a Run that was actually saved there. `run_later()` saves automatically, but only when given a `DurableQueue` (e.g. `SQLiteQueue`) — that's how `recover_pending()` and CLI inspection see background work at all. Neither the default `InlineQueue` nor a plain synchronous `Agent.run()` saves anything — persistence stays outside the core execution primitive (CLAUDE.md) — so either one is only visible to `runa runs show`/`list` if the app calls `default_run_store().save(run)` itself. Either way, it needs a durable store to survive across process boundaries — `runa.configure()`'s own default is in-memory, but the `main.py` `runa new` generates already wires up `run_store=SQLiteRunStore("runa.db")`, so there's no extra setup once a Run is actually saved. `runa runs list [--status STATUS] [--since TIMESTAMP]` answers the question one Run's timeline can't — "what happened across every Run" — by filtering `RunStore.list()` itself: `InMemoryRunStore` filters in Python, `SQLiteRunStore` pushes both filters into the `WHERE` clause using the `status`/`created_at` columns it already keeps outside the JSON blob, so listing doesn't deserialize every row just to discard most of them.
