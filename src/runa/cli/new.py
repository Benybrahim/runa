...
rather than at each call site: swap `OpenAIProvider` for
`AnthropicProvider` (or any other Provider) as this app's needs change.
`provider` is what `Agent.run()`/`.run_sync()`/`.run_stream()`/
`.run_later()` actually drive: Runa's canonical execution model is async
(see docs/concepts.md). `load_dotenv()` reads the key(s) out of `.env`
(see that file) so every `runa` command picks them up without exporting
